...
Mock.mock('/articles/hot', 'post', (options) => {
    // 将评论最少的文章排在前面
    let fiteredArticles = store.getters.getArticlesByFilter('noreply')
    // 将评论最多的文章排在前面
    let articles = fiteredArticles.reverse()
    // 将 7 天内评论最多的文章
    let hotActicles = articles.filter((article) => (new Date() - new Date(article.date) < 604800000))
    ...
    hotActicles = hotActicles.slice(0, num || 10)
